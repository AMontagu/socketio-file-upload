...
test("test setup only one function", function (t) {
	var requestHandler = ecstatic({
		root: __dirname + "/serve",
		cache: 0
	});
	...
	setup.listen(server, function(port){
		return function(){
			...
			t.equal(uploader.topicName, "siofu_only_topic", "instance.topicName to siofu_only_topic");
			t.ok(uploader.onlyOneTopic, "instance.onlyOneTopic is true");
			t.deepLooseEqual(uploader.wrapOptions, {
				siofuDataKey: "data",
				siofuActionKey: "action"
			} ,"instance.wrapOptions are set to wrap data");
			t.deepLooseEqual(uploader.unwrapOptions, {
				siofuDataKey: "message",
				siofuActionKey: "action"
